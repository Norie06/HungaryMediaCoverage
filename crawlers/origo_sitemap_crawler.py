"""Origo sitemap crawler

Fetches monthly sitemaps for Origo across the configured RELEVANT_DATE_RANGE
and extracts <loc> and <lastmod> (date only). Saves results to
`crawlers/origo_sitemap_urls.json` as a list of objects:
...
OUT_FILE = Path(__file__).resolve().parents[0] / "origo_urls.json"
...
def collect_origo_urls(save_path: str = None) -> List[Dict[str, str]]:
    cfg = OUTLET_SETTINGS.get("origo")
    if not cfg:
        raise ValueError("Missing origo settings in media_outlets.py")
    template = cfg.get("sitemap_url_template")
    if not template:
        raise ValueError("Missing sitemap_url_template for origo in media_outlets.py")
