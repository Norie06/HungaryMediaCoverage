"""Mandiner sitemap crawler

Fetches monthly sitemaps for Mandiner across the configured RELEVANT_DATE_RANGE
and extracts <loc> and <lastmod> (date only). Saves results to
`crawlers/mandiner_sitemap_urls.json` as a list of objects:
...
OUT_FILE = Path(__file__).resolve().parents[0] / "mandiner_urls.json"
...
def collect_mandiner_urls(save_path: str = None) -> List[Dict[str, str]]:
    cfg = OUTLET_SETTINGS.get("mandiner")
    if not cfg:
        raise ValueError("Missing mandiner settings in media_outlets.py")
    template = cfg.get("sitemap_url_template")
    if not template:
        raise ValueError("Missing sitemap_url_template for mandiner in media_outlets.py")
